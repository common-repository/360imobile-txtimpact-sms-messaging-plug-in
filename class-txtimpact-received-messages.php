...
class TXTIMPACT_Received_messages {
	...
	public static function check_update() {
		...
		if ( self::VERSION > $version ) {
			error_log( "TXTIMPACT: Upgrading received messages DB table" ); // phpcs:ignore
	...
	public static function fetch_row_by_phone_number( $phone_number ) {
		...
		return $wpdb->get_row( $wpdb->prepare( " SELECT * FROM $table WHERE phone_number = %s ", $phone_number ) ); // phpcs:ignore
	}
	...
	public static function fetch_all( $page = 1, $pageSize = 10, $opt_out = null ) {
		...
		$table = $wpdb->prefix . 'txtimpact_received_messages';
		$where = '';
		$query = $wpdb->prepare( " SELECT * FROM $table WHERE 1=1 $where ORDER BY `created` DESC LIMIT %d, %d ", $start, $pageSize ); // phpcs:ignore
	 ...
	 * @global          $wpdb WordPress Database Object
	 *
	 * @param int|array $message_ids id.
	 *
	 * @return int
	 */
