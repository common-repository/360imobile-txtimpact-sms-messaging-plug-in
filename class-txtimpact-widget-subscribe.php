...
class TXTIMPACT_Widget_Subscribe extends TXTIMPACT_Widget {
	...
	/**
	 * TXTIMPACT_Widget_Subscribe constructor.
	 *
	 * @param bool   $id_base id.
	 * @param string $name name.
	 * @param array  $widget_options options.
	 * @param array  $control_options options.
	 */
	public function __construct( $id_base = false, $name = 'TXTImpact Widget', $widget_options = array(), $control_options = array() ) {
	...
	public function form( $instance ) {
		...
		$this->textarea( __( 'Successful Signup Message', 'txtimpact' ), 'success_info', $success_info, __( 'This message will appear after a customer or member has successfully been added to your text messaging list', 'txtimpact' ) );
	}
	...
	public function widget( $args, $instance ) {
		...
		?><?php echo $before_widget; ?><?php if ( $title ) : ?><?php echo $before_title . $title . $after_title; ?><?php endif; ?><?php if ( $info ) : ?>
		...
		<form action="<?php echo esc_attr( admin_url( 'admin-ajax.php' ) ); ?>" method="post" class="txtimpactsubscribe_form">
			<input type="hidden" name="action" value="txtimpact_subscribe" id="action"> <input type="hidden" name="successInfo" value="<?php echo esc_attr( $success_info ); ?>"/>
			<p class="txtimpactsubscribe-phoneNumber">
				<span><?php esc_html_e( 'Enter your mobile number (Country Code Included):', 'txtimpact' ); ?></span> <input type="text" name="phone_number" value="" id="<?php echo esc_attr( $widget_id ); ?>"> <span
					class="phoneNumber-error"></span><br/> <span class="infos">Msg&Data rates may apply</span>
			</p>
			<p class="txtimpact-subscribe">
				<input type="submit" name="subscribe_button" value="<?php echo esc_attr( __( 'Subscribe', 'txtimpact' ) ); ?>"/> |
				<a href="<?php echo esc_attr( add_query_arg( array( 'txtimpact-unsubscribe' => 1 ), home_url() ) ); ?>"><?php esc_html_e( 'Unsubscribe', 'txtimpact' ); ?></a>
			</p>
		</form>
...
add_action( 'widgets_init', function () {
	register_widget( 'TXTIMPACT_Widget_Subscribe' );
} );
