...
class TXTIMPACT extends TXTIMPACT_Plugin {
	...
	public function manage_options() {
		...
		if ( empty( $security_key ) || ! empty( $new_var ) ) {
			...
			echo "<script type='text/javascript'>top.location.href = '" . esc_url( get_admin_url() ) . "admin.php?page=txtimpact_options';</script>";
		}
	...
	public function load_send_message() {
		...
		if ( $response->failed ) {
			if ( isset( $response->response['errors']['301'] ) ) {
				$this->set_admin_error( __( $response->response['errors']['301']['message'] . '! Your txtimpact username, password and/or vasid are missing or incorrect.', 'txtimpact' ) . ' <a href="' . get_admin_url() . 'admin.php?page=txtimpact_options">' . __( 'Please visit the settings page', 'txtimpact' ) . '</a>' ); // phpcs:ignore
	...
	protected function delete_uninstall() {
		...
		$wp_function->wp_die( sprintf( __( "txtimpact Texting: The SMS notifications plugin has been deactivated and all its data has been deleted; return to the <a href='%s'>Dashboard</a>.", 'txtimpact' ), admin_url() ), __( 'Confirm uninstall and delete!', 'txtimpact' ), array( 'response' => 200 ) ); // phpcs:ignore
	}
